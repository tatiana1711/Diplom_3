...
class OrdersPage(BasePage):
    ...
    def _find_order_by_number(self, order_number, timeout=15):
        ...
        # используется в find_order_in_feed_by_number и find_order_in_progress_by_number
        try:
            # ждем загрузки ленты заказов
            self.wait_until(EC.presence_of_element_located(OrderFeedLocators.ORDERS_IN_FEED), timeout)
            
            # ищем заказ по номеру
            order_locator = self.get_order_locator_by_number(order_number)
            element = self.wait_until(EC.presence_of_element_located(order_locator), timeout)
    ...
    def wait_for_total_orders_increase(self, initial_count, timeout=15):
        ...
        self.wait_until(lambda driver: self.get_total_orders_count() > initial_count, timeout)
    ...
    def wait_for_today_orders_increase(self, initial_count, timeout=15):
        ...
        self.wait_until(lambda driver: self.get_today_orders_count() > initial_count, timeout)
